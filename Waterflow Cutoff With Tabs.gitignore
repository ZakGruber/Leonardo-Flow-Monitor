...
"""
Calculations based on flow rates and sensor differences
"""
...
## post reset processing function
...
## turning off the system (records as a shutoff date)
...
##Graphs Tab
        with dpg.tab(label="Graph Panel"):
            ...
            def update_graph():
                ...
                if series_id and time_stamps and sensor_differences:
                    min_time = min(time_stamps) if time_stamps else time.time()
                    normalized_timestamps = [t - min_time for t in time_stamps]  # Keeps relative scale

                    dpg.set_value(series_id, [normalized_timestamps, sensor_differences])

                    # Ensure that the latest data is always on the right
                    max_x_value = 10
                    dpg.set_axis_limits(x_axis, max_x_value - 10, max_x_value)
